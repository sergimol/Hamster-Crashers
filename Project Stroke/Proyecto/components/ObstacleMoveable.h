...
public:
	ObstacleMoveable(Texture* wTx, float x, float y, float z, int floor, int scale, float velo,
		float spawTime 
		
		
		) : lastTime_(0), spawnTime_(spawTime), lifeTime_(3000), time_(0),
		warningTime_(2000), warningTimeFlickering_(100),
		spawns_(1), warningTx(wTx),
		obstacle(nullptr), vel(velo), x_(x), y_(y), z_(z), floor_(floor),
